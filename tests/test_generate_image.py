...
class TransportNetworkErrorTests(unittest.TestCase):
    ...
    TimeoutError; the transport must convert it so poll retries and the
    controlled error path apply instead of crashing."""

    def test_request_read_timeout_becomes_controlled_error(self):
        transport = CLIENT.UrlLibTransport(request_timeout=1.0)
        for method, payload, stage in (
            ("POST", {"prompt": "x"}, "submit"),
            ("GET", None, "poll"),
        ):
            with mock.patch.object(
                CLIENT, "urlopen", side_effect=TimeoutError("timed out")
            ):
                with self.assertRaises(CLIENT.RightCodeError) as ctx:
                    transport.request_json(
                        method, CLIENT.SUBMIT_URL, "test-key", payload, stage=stage
                    )
            self.assertIn("network error", str(ctx.exception))
            self.assertIn("timed out", str(ctx.exception))

    def test_download_read_timeout_becomes_controlled_error(self):
        transport = CLIENT.UrlLibTransport(request_timeout=1.0)
        with mock.patch.object(CLIENT, "urlopen", side_effect=TimeoutError("timed out")):
            with self.assertRaises(CLIENT.RightCodeError) as ctx:
                transport.download("https://www.rightapi.ai/file.png", "test-key")
        self.assertIn("download network error", str(ctx.exception))

    def test_poll_retries_after_transport_timeout(self):
        class TimeoutThenSuccessTransport:
            ...
            def request_json(self, method, url, api_key, payload=None, stage="request"):
                ...
                self.polls += 1
                if self.polls == 1:
                    raise CLIENT.RightCodeError(
                        "Right Code poll network error: GET /v1/tasks/task-timeout -> timed out"
                    )
                encoded = base64.b64encode(b"\x89PNG\r\n\x1a\nmock").decode("ascii")
                return {"status": "completed", "data": [{"b64_json": encoded}]}
        ...
        with tempfile.TemporaryDirectory() as temporary:
            result = CLIENT.generate(
                api_key="test-key",
                payload={"model": "gpt-image-2.5", "prompt": "retry", "n": 1, "async": True},
                output_dir=Path(temporary),
                poll_interval=0,
                timeout=30,
                poll_retries=5,
